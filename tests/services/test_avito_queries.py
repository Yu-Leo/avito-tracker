...
class TestAvitoQueryService:
    ...
    def test_create(self, database_session):
        # Arrange
        ...
        # Assert
        object_from_db = database_session.query(models.AvitoQuery).get(created_object.id)
        assert created_object == object_from_db

    def test_get_list(self, database_session, avito_queries_list):
        # Arrange
        avito_query_service = AvitoQueryService(database_session)
...
class TestAvitoQueryValueService:
    ...
    def test_create(self, database_session):
        # Arrange
        avito_query_value_service = AvitoQueryValueService(database_session)
        ...
        # Assert
        object_from_db = database_session.query(models.AvitoQueryValue).get(created_object.id)
        assert created_object == object_from_db
    ...
    @pytest.mark.parametrize('avito_query_id, start, end', [(1, None, None),
                                                            ...
                                                            (2, None, None), ])
    def test_get_by_avito_query_id(self, avito_query_id, start, end, database_session, avito_query_values_list):
        # Arrange
        avito_query_value_service = AvitoQueryValueService(database_session)
